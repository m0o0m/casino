...
var casinoTitle="";function setGameSetting(doRes,gameW,gameH,gameID,title)
{if(maxWidth!=gameW&&maxHeight!=gameH)
{doSetRes=(doRes.toLowerCase()=="true");maxWidth=gameW;maxHeight=gameH;if(siteSettings.allowResolutionLocking){if(doSetRes){$('#system').attr({width:'100%',height:'100%'});$('#system').css({marginLeft:'0px',marginTop:'0px'});}
setFlashResolution();}}
if(gameID!=undefined)
...
if(siteSettings.allowResolutionLocking&&(!siteSettings.useRMM)&&(!siteSettings.useOpenId))
{$(window).resize(function()
{setFlashResolution();setJITMigrationListener();});}
else if(siteSettings.allowResolutionLocking&&siteSettings.useRMM&&(!siteSettings.useOpenId))
{$(window).resize(function()
{setFlashResolution();resizeRMM();setJITMigrationListener();});}
else if(siteSettings.allowResolutionLocking&&siteSettings.useRMM&&siteSettings.useOpenId)
{$(window).resize(function()
{setOpenIDResizeListener();setFlashResolution();resizeRMM();setJITMigrationListener();});}
else if((!siteSettings.allowResolutionLocking)&&siteSettings.useRMM&&siteSettings.useOpenId)
{$(window).resize(function()
{setOpenIDResizeListener();resizeRMM();setJITMigrationListener();});}
else if((!siteSettings.allowResolutionLocking)&&(!siteSettings.useRMM)&&siteSettings.useOpenId)
{$(window).resize(function()
{setOpenIDResizeListener();setJITMigrationListener();});}
var siteLaunchSettings=new Object();siteLaunchSettings.pleaseWaitText=aspStrings.PLEASE_WAIT_TEXT;siteLaunchSettings.queryString=urlSettings.queryString;siteLaunchSettings.safeQueryString=urlSettings.safeQueryString;siteLaunchSettings.killFramesetOnRelocate=startupSettings.killFramesetOnRelocate;siteLaunch=new SiteLauncher(siteLaunchSettings);$.jSignal.connect(siteLaunch,siteLaunch.siteClosed,javaScriptRoot,javaScriptRoot.externalSiteClosed);$.jSignal.connect(siteLaunch,siteLaunch.siteBlocked,javaScriptRoot,javaScriptRoot.popupBlocked);$.jSignal.connect(siteLaunch,siteLaunch.blockedSiteOpened,javaScriptRoot,javaScriptRoot.blockedPopupOpened);if(siteSettings.faxControlAllowed)
...
{onFailedActivation('Need to install the Adobe Flash Player.');}}});$("#contentFrame").css({width:'100%',height:'100%',overflow:'auto',float:'left'});$("#contentFrame").css('min-width','800px');$("#contentFrame").css('min-height','600px');redirect();}}});function placeFSControl(id,domain,divId)
...
{rmmAnimating=1;var width=$(window).width();$("html").css("overflow","hidden");$('body').append('<iframe id="RMM" name="RMM" scrolling="no" src="javascript:false" style="border:0px; padding:0px; margin:0px; z-index:4; position:absolute; overflow:hidden; top:0px; left: '+width+'px; height:0px; width:0px; opacity:0"></iframe>');}
function focusRMMessage()
{if($('#RMM').length==1)
{$('#RMM')[0].focus();}}
function closeRMMessage()
{rmmAnimating=0;rmmShowing=0;var width=$(window).width();$('#RMM').css({"width":0,"height":0});$('#RMM').remove();$("#system")[0].rmmClosed();}
...
function RMMCentredFanfair(width,height)
{currentRMMheight=height;currentRMMwidth=width;rmmShowing=1;$('#RMM').css({"width":currentRMMwidth,"height":currentRMMheight,"top":getCentredFanfairTOP(height),"left":getCentredFanfairLEFT(width),opacity:0,"overflow":"hidden"});$('#RMM').animate({opacity:1},RMM_CENTERED_DISPLAYTIME,"linear",RMMsetResizeListener);rmmPlacement=RMM_PLACED_CENTRE;return RMM_CENTERED_DISPLAYTIME;}
...
switch(rmmPlacement)
{case RMM_PLACED_CENTRE:{$('#RMM').css({"width":currentRMMwidth,"height":currentRMMheight,"top":getCentredFanfairTOP(currentRMMheight),"left":getCentredFanfairLEFT(currentRMMwidth)});break;}
...
throw new SyntaxError('JSON.parse');};}}());var easyXDMSocket;$(document).ready(function(){try{easyXDMSocket=new easyXDM.Socket({swf:"easyxdm/easyxdm.swf",onMessage:function(message,origin){try{if(message=="[balance]"){$("#system")[0].getMainBalance();}
else if(message=="[close]"){$("#system")[0].externalExit();}}catch(error){};}});}catch(error){};setLoader();});function setLoader(width,height){try{var obj=new Object();obj.action="SETLOADER";var jsonString=JSON.stringify(obj);easyXDMSocket.postMessage(jsonString);}catch(error){};}
...
var maxWidth=1024;var maxHeight=768;function setLobbySetting(lobbyW,lobbyH){maxWidth=lobbyW;maxHeight=lobbyH;if($(window).width()>maxWidth&&$(window).height()>maxHeight){if($('#system').width()!=maxWidth)
